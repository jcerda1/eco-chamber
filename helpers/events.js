...
const getTopTenEvents = (category, date) => {
  ...
  const q = new QueryEventsIter(er, {
    ...
    dateStart: date,
    sortBy: 'size',
    sourceUri: QueryItems.AND(sourcesAll),
    maxItems: 10,
...
const buildSaveConceptOrCategory = (obj, type) => {
  ...
  return model.find({where: {uri: obj.uri}}).then(result => {
      if (result === null) {
        ...
        console.log(`this ${type} already exists:`);
        return result;
      }
  });
}
...
const associateConceptsOrCategories = async (conceptsOrCategories, type, eventUri) => {
  ...
  for (const item of conceptsOrCategories) {
    ...
    console.log(saved.dataValues, event.dataValues);

    if (type === 'concept') {
      event.addConcept(saved).then(result => console.log(result));
    } else if (type === 'category') {
      event.addCategory(saved).then(result => console.log(result));
    }
  }
}
